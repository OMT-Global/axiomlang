...
def run(root):
    return subprocess.run([sys.executable, str(root / "scripts/ci/check-semantic-mir-v1.py")], cwd=root, env=checker_env(root), text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
