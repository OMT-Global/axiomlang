...
def run(root: Path) -> subprocess.CompletedProcess[str]:
    return subprocess.run([sys.executable, str(root / "scripts/ci/check-runtime-associative-collections-v1.py")], cwd=root, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def main() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        if run(root).returncode == 0:
            raise SystemExit("duplicate associative collection operations were accepted")
        ...
        path.write_text(json.dumps(value))
        if run(root).returncode == 0:
            raise SystemExit("incomplete associative collection fixtures were accepted")
        value = json.loads((ROOT / "stage1/compiler-contracts/snapshots/runtime-associative-collections-v1.json").read_text())
        value["equality"]["hash_visibility"] = "Rust HashMap detail"
        path.write_text(json.dumps(value))
        if run(root).returncode == 0:
