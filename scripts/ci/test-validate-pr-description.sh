...
BODY
    ...
    dependabot_body_valid)
      body=$(cat <<'BODY'
Bumps [anthropics/claude-code-action](https://github.com/anthropics/claude-code-action) from 1.0.111 to 1.0.117.

Release notes are generated by Dependabot.
BODY
)
      ;;
...
BODY
  ...
  set +e
  if [[ "$case_name" == "dependabot_body_valid" ]]; then
    PR_AUTHOR="dependabot[bot]" PR_BODY="$body" bash "$source_script" >"$output_path" 2>&1
  else
...
run_case dependabot_body_valid success
