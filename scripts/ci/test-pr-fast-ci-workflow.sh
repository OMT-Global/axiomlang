...
if [[ -z "$runtime_process_check" ]]; then
  echo "trusted fast checks must validate Runtime Process v1 evidence from the PR-head checkout root" >&2
  exit 1
fi

if [[ -z "$runtime_process_regression" ]]; then
  echo "trusted fast checks must retain the trusted Runtime Process v1 checker self-tests" >&2
  exit 1
fi
