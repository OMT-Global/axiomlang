...
behind `compiler.commands` package APIs.
...
The typed Documentation v1 slice adds compiler-owned public-symbol extraction
to `project.rs` and deterministic document, link, search, doctest, and HTML
link-rendering orchestration to `main.rs`. The ceiling update below records
that bounded feature growth explicitly. The next extraction for this surface is a
`compiler.docs` module that can own rendering and qualification policy without
making Rust module paths canonical; this PR keeps the compiler semantic source
...
| 4 | `stage1/crates/axiomc/src/codegen.rs` | 7,680 | `compiler.backend.generated_rust`, `compiler.backend.contracts` | Isolate generated-Rust compatibility emission from backend target selection and unsupported-feature contracts. |
| 5 | `stage1/crates/axiomc/src/syntax.rs` | 6,396 | `compiler.syntax`, `compiler.diagnostics` | Split lexer/parser, parse recovery, source spans, macros, and syntax diagnostics behind the syntax boundary. |
...
| `stage1/crates/axiomc/src/project.rs` | 14014 |
| `stage1/crates/axiomc/src/build_contract.rs` | 118 |
| `stage1/crates/axiomc/src/main.rs` | 11963 |
...
| `stage1/crates/axiomc/src/hir/reachability.rs` | 162 |
| `stage1/crates/axiomc/src/hir/signatures.rs` | 471 |
| `stage1/crates/axiomc/src/hir/source_locations.rs` | 91 |
...
| `stage1/crates/axiomc/src/lib.rs` | 39 |
| `stage1/crates/axiomc/src/lsp.rs` | 3216 |
