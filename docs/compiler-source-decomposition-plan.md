...
`stage1/crates/axiomc/src/hir/ownership.rs`, lowering both absolute top-file
lines and share.
...
| 3 | `stage1/crates/axiomc/src/hir.rs` | 8,716 | `compiler.hir` | Generic inference and monomorphization now live in `stage1/crates/axiomc/src/hir/generics.rs`; public HIR model types now live in `stage1/crates/axiomc/src/hir/model.rs`; syntax-to-HIR type/literal lowering now lives in `stage1/crates/axiomc/src/hir/types.rs`; type-name, aggregate, and trait-use definition checks now live in `stage1/crates/axiomc/src/hir/definitions.rs`; function/method signatures and trait impl signature validation now live in `stage1/crates/axiomc/src/hir/signatures.rs`; capability analysis now lives in `stage1/crates/axiomc/src/hir/capabilities.rs`; expression typing helpers now live in `stage1/crates/axiomc/src/hir/expressions.rs`; ownership and borrow-state helpers now live in `stage1/crates/axiomc/src/hir/ownership.rs`; next split property clauses and HIR diagnostics behind the package APIs in `docs/compiler-hir-ownership-capability.md`. |
...
| --- | ---: |
| `summary.top_file_line_share` | 0.8587 |
| `summary.top_file_lines` | 76533 |
| `stage1/crates/axiomc/src/cranelift_backend.rs` | 27994 |
| `stage1/crates/axiomc/src/hir.rs` | 8716 |
...
3. `compiler.hir`: generic inference/monomorphization, public HIR model types,
   ...
   capability analysis, expression typing helpers, and ownership/borrow helpers
   are split; continue with property checks so diagnostics stay stable.
