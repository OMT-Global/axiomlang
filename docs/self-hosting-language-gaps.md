...
| 10 | No executable `for` iteration protocol; the target semantics and fail-closed evidence floor are published in `docs/iteration-control-v1.md` | existing conformance fixture `for_loop_requires_iteration_protocol` | Style only until runtime-sized compiler collections land. | `while` + index. | Low; the contract gate prevents a syntax-only implementation from being reported as runtime-ready. |
| 11 | ~~No mutable array bindings~~ **Partially shipped**: fixed arrays with scalar elements can be updated by index (see `stage1/conformance/pass/local_array_index_assignment`) | previously: `assignment target must be a scalar local, dereference a mutable reference, or index a mutable slice, got int` | Local fixed-array scratch buffers can now be updated directly for scalar rows. | Residual: non-scalar element arrays still require helper patterns until aggregate element writes are lowered. | Was Medium; residual Low-Medium. |
