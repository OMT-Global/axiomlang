...
|---|---|---|---|---|---|
| 1 | ~~No comment syntax of any kind~~ **Shipped**: full-line `//` comments (see `stage1/conformance/pass/line_comments`) | previously: `stage1 bootstrap currently supports top-level import, const, static, ...` | Spike sources now carry boundary comments. | Full-line comments only; trailing `// ...` after code is still unsupported and stays open under this gap. | Was High; residual (trailing comments) Low. |
