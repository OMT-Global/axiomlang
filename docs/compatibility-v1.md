...
Package Trust v1 schemas, eleven additive base-contract schemas (Compiler Syntax Migration v1, Filesystem v1,
...
`before-syntax-migration-v1/contract.json` freezes the 73-surface `0.9.0` source
contract at main commit `351493a02a59cd8f9f970ff6513fdfc18be5032b`. The `0.10.0`
one-step ratchet adds only the compiler syntax migration schema at `0.1.0`.
Historical snapshots remain unchanged; this is source evidence, not a qualified
previous compiler.
