...
branch returns and branch-condition comparisons from direct fixed-array
helper-call indexes.
Fixed-array `len`,
...
through the same nested tag/payload slot representation. The Cranelift evidence
suite now also builds and runs public `std/outcome.ax`
