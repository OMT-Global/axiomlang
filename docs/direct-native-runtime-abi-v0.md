...
materializing a general byte-array value. Those native byte-length reads append
best-effort host audit JSONL to `AXIOM_HOST_AUDIT_LOG`, recording the typed
requested length and outcome without recording generated bytes. A package
without the `crypto` capability still fails before backend lowering.
Direct-native `random_u64()` runtime value semantics, `random_bytes(...)` byte
storage and contents, portable entropy source parity, deterministic test hooks,
and broader runtime audit parity remain open under #1001.
