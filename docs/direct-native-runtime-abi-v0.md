...
and function returns. The public borrowed-slice smoke asserts
`generated_rust: null` while printing `len`, `first`, `last`, and index
projections from both direct local borrowed slices and helper-returned borrowed
slices. The direct-native runtime path now also lowers narrow static-range
fixed-array slices using literal or static scalar bounds such as `values[1:]`,
