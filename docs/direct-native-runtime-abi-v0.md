...
Known-input `std/serdes` JSON object, stringified text, parsed text, and
parse-error string results can also feed terminal panic reports as escaped
native stderr JSON while preserving `generated_rust: null`. Broader dynamic
...
stringified text, parsed text, and parse-error strings can also feed terminal
panic reports as escaped native stderr JSON without generated Rust. Terminal
