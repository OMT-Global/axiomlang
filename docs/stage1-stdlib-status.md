...
| #234 net sockets | `std/net.ax` supports DNS resolution, HTTP client GET exists in `std/http.ax`, `std/net_tcp.ax` exposes blocking loopback TCP listener/stream handles with read/write/close operations, and `std/net_udp.ax` exposes loopback UDP bind/send/recv handles. Raw socket bind and peer literals honor `[capabilities].net.hosts` and `[capabilities].net.ports`. | Deeper async runtime integration remains open. |
