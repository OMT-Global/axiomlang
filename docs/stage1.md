...
- Phase H.1 property clauses are accepted as `property fn name(input: T): bool { ... }`. A property clause is a typed boolean invariant over one input; the checker rejects generic, multi-input, or non-`bool` property signatures and reports `property_failed` when the verdict is statically false. Boolean `&&` and `||` expressions can compose verdicts, and `return assert_true(<bool>)` is accepted as property verdict syntax. Sampled property runners remain separate test-runner work.
...
`--json` adds stable package membership plus golden-output and compile-fail
...
- Structs, tuples, tuple-style enum payloads, named-payload enum variants, `Option<T>`, `Result<T, E>`, maps, arrays, borrowed slice types, borrowed array slice expressions, borrowed slices stored inside named structs and enum payloads, borrowed-return aggregates backed by one or more borrowed parameters, field access, tuple indexing, map indexing, array indexing, exhaustive statement-level `match`, monomorphized generic functions, generic structs, generic enums, and the built-in collection helpers `len(...)`, `first(...)`, and `last(...)` now exist, but there is still no general borrow system. `len(...)` accepts arrays, slices, `string`, and `&str`; for strings it returns the encoded byte length used by protocol framing surfaces such as JSON-RPC `Content-Length`.
- `const fn` declarations are parsed and lowered with a stage1 purity fence: bodies may use `let`, `if` / `else`, `return`, pure arithmetic/comparison expressions, and calls to other `const fn`; calls into host-runtime intrinsics such as `clock_now_ms` are rejected before normal capability lowering. Evaluating `const fn` calls inside scalar `const` initializers remains future #734 work.
- No inferred generic function, struct, or enum type arguments.
- No closures, inferred trait bounds, associated items, blanket impls, supertraits, or dynamic trait dispatch. Static impl methods and bounded generic trait calls exist for local concrete structs/enums; `async fn` and `await` exist for stage1 `Task<T>` values; spawned tasks run through a generated host scheduler pool sized by `[runtime].max_threads` or host parallelism, sleeps share one generated timer wheel, and timeouts use bounded scheduler waits.
...
- Borrowed slices can now flow through direct `&[T]` returns, named structs, enum payloads, and aggregate return types like `Option<&[T]>` or tuples when they are derived from a single borrowed parameter; functions with multiple borrowed parameters returning a borrow are rejected until explicit origin annotation syntax lands. `Option` / `Result` match bindings preserve enough borrow provenance to return those borrowed payloads again, statically false control-flow is now skipped instead of contaminating move state, and live borrowed slices now block later owner moves until their scope ends even when those borrows are stored inside local aggregate wrappers, named structs, enum payloads, or temporary `match` / call expressions, but there are still no general borrows, mutable borrows, full lifetime checks, or precise path-sensitive borrow narrowing beyond constant conditions.
...
- The stdlib surface now covers every stage1 capability-gated intrinsic with a thin wrapper module (`std/time.ax`, `std/env.ax`, `std/fs.ax`, `std/net.ax`, `std/process.ax`, `std/crypto_hash.ax`, `std/crypto_mac.ax`), plus `std/http.ax` (capability-gated HTTP client helper `http_get`, loopback server handle helpers over `http_server_*`, request metadata helpers over `http_request_*`, response writing over `http_response_write`, blocking single-request server helper `http_serve_once`, and bounded route-based helper `http_serve_route` sharing the existing `net` surface), `std/http_async.ax` (async-gated bounded HTTP route serving), `std/io.ax` (first ungated stdlib module, `eprintln` on top of the new `io_eprintln` intrinsic), `std/json.ax` (ungated scalar/string JSON helpers plus manual object/field builders), `std/serdes.ax` (native `Value` union round-tripped through JSON text with typed scalar, array, object, and field accessors), `std/collections.ax` (generic borrowed-slice helpers built on AG2 generic functions), `std/string_builder.ax` (owned string accumulator), `std/log.ax` (deterministic JSON-line logging over stderr), `std/sync.ax` (ownership-shaped mutex guards, one-shot cells, and single-slot nonblocking channels), `std/async.ax` (task, join, channel, cancellation, bounded timeout, select, and host scheduler pool), and `std/regex.ax` (linear-time matching helpers for common regex constructs). The `net` socket floor is intentionally loopback-only in stage1: the one-shot TCP and UDP listen helpers bind `127.0.0.1:0`, dial/send helpers reject non-loopback targets, HTTP service helpers reject non-loopback bind addresses, payloads are bounded to 64 KiB, and timeouts are clamped to 1-30000 ms. Manifest `net.hosts` and `net.ports` allowlists guard the current outbound TCP/UDP peer arguments; arbitrary listener/bind-address allowlists remain tied to the future raw socket API because the current TCP and UDP one-shot bind helpers do not accept caller-selected bind addresses. `[runtime].max_threads` configures the generated async scheduler pool, defaulting to `std::thread::available_parallelism()`. `std/time` and `std/async_time` sleeps share one generated timer wheel, while `std/async` timeout uses a bounded scheduler wait so timed-out host work does not extend the caller's wait. The `fs` capability is scoped: `fs_read` resolves relative paths from the package root, bounds them to the package root by default or `[capabilities] fs_root = "<relative package path>"`, canonicalizes targets to reject traversal and symlink escapes, and refuses files larger than 64 MiB.

- The stdlib surface now covers every stage1 capability-gated intrinsic with a thin wrapper module (`std/time.ax`, `std/env.ax`, `std/fs.ax`, `std/net.ax`, `std/process.ax`, `std/crypto_hash.ax`, `std/crypto_mac.ax`), plus `std/http.ax` (first stdlib module with a brand-new capability-gated intrinsic `http_get` sharing the existing `net` surface), `std/io.ax` (first ungated stdlib module, `eprintln` on top of the new `io_eprintln` intrinsic), `std/json.ax` (ungated scalar/string JSON helpers plus manual object/field builders), `std/collections.ax` (generic borrowed-slice helpers built on AG2 generic functions), `std/string_builder.ax` (owned string accumulator), `std/log.ax` (deterministic JSON-line logging over stderr), `std/sync.ax` (ownership-shaped mutex guards, one-shot cells, and single-slot nonblocking channels), and `std/async.ax` (task, join, channel, cancellation, timeout, select, scheduler pool, and timer-wheel backed sleep wrappers). The `net` socket floor is intentionally loopback-only in stage1: the one-shot TCP and UDP listen helpers bind `127.0.0.1:0`, dial/send helpers reject non-loopback targets, payloads are bounded to 64 KiB, and timeouts are clamped to 1-30000 ms. The `fs` capability is scoped: `fs_read` resolves relative paths from the package root, bounds them to the package root by default or `[capabilities] fs_root = "<relative package path>"`, canonicalizes targets to reject traversal and symlink escapes, and refuses files larger than 64 MiB.
- The stdlib surface now covers every stage1 capability-gated intrinsic with a thin wrapper module (`std/time.ax`, `std/env.ax`, `std/fs.ax`, `std/net.ax`, `std/process.ax`, `std/crypto_hash.ax`, `std/crypto_mac.ax`), plus `std/http.ax` (first stdlib module with a brand-new capability-gated intrinsic `http_get` sharing the existing `net` surface), `std/io.ax` (first ungated stdlib module, `eprintln` on top of the new `io_eprintln` intrinsic), `std/json.ax` (ungated scalar/string JSON helpers plus manual object/field builders), `std/collections.ax` (generic borrowed-slice helpers built on AG2 generic functions), `std/string_builder.ax` (owned string accumulator), `std/log.ax` (deterministic JSON-line logging over stderr), `std/sync.ax` (ownership-shaped mutex guards, one-shot cells, and single-slot nonblocking channels), `std/async.ax` (task, join, channel, cancellation, timeout, select, scheduler pool, and timer-wheel backed sleep wrappers), and `std/regex.ax` (linear-time matching helpers for common regex constructs). The `net` socket floor is intentionally loopback-only in stage1: the one-shot TCP and UDP listen helpers bind `127.0.0.1:0`, dial/send helpers reject non-loopback targets, payloads are bounded to 64 KiB, and timeouts are clamped to 1-30000 ms. The `fs` capability is scoped: `fs_read` resolves relative paths from the package root, bounds them to the package root by default or `[capabilities] fs_root = "<relative package path>"`, canonicalizes targets to reject traversal and symlink escapes, and refuses files larger than 64 MiB.

- The stdlib surface now covers every stage1 capability-gated intrinsic with a thin wrapper module (`std/time.ax`, `std/env.ax`, `std/fs.ax`, `std/net.ax`, `std/process.ax`, `std/crypto_hash.ax`, `std/crypto_mac.ax`), plus `std/http.ax` (first stdlib module with a brand-new capability-gated intrinsic `http_get` sharing the existing `net` surface), `std/io.ax` (first ungated stdlib module, `eprintln` on top of the new `io_eprintln` intrinsic), `std/json.ax` (ungated scalar/string JSON helpers plus manual object/field builders), `std/collections.ax` (generic borrowed-slice helpers built on AG2 generic functions), `std/string_builder.ax` (owned string accumulator), `std/log.ax` (deterministic JSON-line logging over stderr), `std/sync.ax` (ownership-shaped mutex guards, one-shot cells, and single-slot nonblocking channels), and `std/async.ax` (task, join, channel, cancellation, timeout, select, scheduler pool, and timer-wheel backed sleep wrappers). The `net` socket floor is intentionally loopback-only in stage1: the one-shot TCP and UDP listen helpers bind `127.0.0.1:0`, dial/send helpers reject non-loopback targets, payloads are bounded to 64 KiB, and timeouts are clamped to 1-30000 ms. The `fs` capability is scoped: `fs_read` resolves relative paths from the package root, bounds them to the package root by default or `[capabilities] fs_root = "<relative package path>"`, canonicalizes targets to reject traversal and symlink escapes, and refuses files larger than 64 MiB.

- Capability-aware integration is now in place for the current stage1 runtime surface: compiler-known intrinsics enforce all six manifest flags, stdlib wrappers preserve that enforcement against the importing package's manifest, capability-denied programs fail before native execution, and the Rust suite covers cross-package capability interactions (`dependency_package_must_enable_its_own_capabilities`) plus per-wrapper denial paths. `std/async_net.ax` shares the same `net` capability gate as `std/net.ax`; current async net helpers wrap the bounded loopback TCP/UDP intrinsics as `Task`-returning calls and expose raw TCP listener accept plus owned-string recv/send helpers for connection-per-task services. Borrowed-buffer async recv/send remains future runtime work because spawned tasks cannot carry caller-owned `&mut [u8]` buffers across host-thread boundaries.
- A host-thread scheduler pool, condvar-backed channel wakeups, and a shared timer wheel now back the generated async runtime. The current runtime can support bounded loopback socket and HTTP service slices, while readiness-based reactor I/O remains future work.
