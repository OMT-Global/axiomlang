...
- `stage1/examples/stdlib_crypto_mac` extends AG4.1 with `import "std/crypto_mac.ax"`, bringing `hmac_sha256(key, message)`, `hmac_sha512(key, message)`, `verify_sha256(tag, key, message)`, `verify_sha512(tag, key, message)`, and `constant_time_eq(left, right)` into scope and staying subject to the importing package's `[capabilities] crypto` flag. `std/crypto.ax` is the umbrella import for the landed hash and MAC helpers. Constant-time comparison currently covers string tags; raw byte-buffer comparison remains future crypto surface.
