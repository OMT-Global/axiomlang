...
- No closures, inferred trait bounds, associated items, blanket impls, supertraits, or dynamic trait dispatch. Static impl methods and bounded generic trait calls exist for local concrete structs/enums; `async fn` and `await` exist for stage1 `Task<T>` values, but the runtime is deterministic and does not yet provide host-thread scheduling.
...
- Remaining AG4.2 runtime work is a configurable host-thread pool, readiness-based I/O wakeups, and a shared timer wheel for large fan-out workloads; the current async runtime can spawn host tasks and block on channels/socket calls, but it is not yet a service-grade reactor.
