...
- Integer overflow follows the generated Rust build mode: debug native builds panic on ambient overflow, while optimized release builds use the target backend's release overflow behavior. User code that needs explicit semantics should prefer the integer helpers `wrapping_add`, `checked_add`, and `saturating_add`; checked helpers return `Option<T>`, and all helper arguments must match the receiver width exactly.
