...
- `std/io.ax` exposes ambient stdio helpers without a manifest capability: `eprintln(text): int`, `readline(): Option<string>`, and `read_to_string(): string`. `readline()` strips one trailing line ending and returns `None` at EOF; `read_to_string()` reads stdin until EOF and returns an empty string when stdin is already closed or empty.
