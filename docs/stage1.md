...
- No methods, trait-style interfaces, or closures. `async fn` and `await` exist for stage1 `Task<T>` values; spawned tasks run through a generated host scheduler pool sized by `[runtime].max_threads` or host parallelism, and sleeps/timeouts share one generated timer wheel.
...
- A host-thread scheduler pool, condvar-backed channel wakeups, and a shared timer wheel now back the generated async runtime. Richer service-grade I/O is still represented by the current loopback-only HTTP and socket slices.
