...
- No methods, trait-style interfaces, or closures. `async fn` and `await` exist for stage1 `Task<T>` values; spawned tasks run through a generated host scheduler pool sized by `[runtime].max_threads` or host parallelism, sleeps share one generated timer wheel, and timeouts use bounded scheduler waits.
