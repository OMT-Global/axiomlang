...
| 4 | `compiler.hir` | `hir.rs`, `borrowck.rs` | Typed declarations, name resolution, imports, capability checks, ownership and borrow validation, property clauses. The HIR package APIs are defined in [Compiler HIR, Ownership, and Capability Package](compiler-hir-ownership-capability.md). | Daedalus | `make stage1-hir-boundary` and `make stage1-compiler-property-test` | High: do not define concepts by Rust lifetimes, traits, `Option`, `Result`, or borrow checker implementation shortcuts. |
| 5 | `compiler.mir` | `mir.rs` | Lowered executable compiler IR, control flow, intrinsic calls, test/property entrypoints, backend input. The MIR-to-backend package APIs are defined in [Compiler MIR and Backend Packages](compiler-mir-backend-packages.md). | Daedalus | `make stage1-mir-backend-boundary` and `cargo test --manifest-path stage1/Cargo.toml -p axiomc mir` | Medium: MIR is an implementation layer, not Intent IR; keep semantic graph mapping explicit. |
| 6 | `compiler.stdlib` | `stdlib.rs`, `stage1/stdlib/std/*.ax` | Embedded standard library modules, capability-gated stdlib surfaces, stdlib property fixtures. Capability/effect and provider authority is defined by the [Compiler stdlib catalog authority](compiler-stdlib-catalog.md); the capability ledger is downstream parity evidence. | Daedalus | `python3 scripts/ci/check-stdlib-catalog.py --json`, `python3 scripts/ci/test-check-stdlib-catalog.py`, and `make stage1-stdlib-test` | Medium: Rust intrinsics are backend shims; stdlib contracts must be described in AxiOM terms. |
| 7 | `compiler.backend.contracts` | `backend-target-interface-v0.md`, `direct-native-runtime-abi-v0.md`, backend selection in `codegen.rs` and `project.rs` | Target selection, supported feature/effect declarations, artifact kinds, unsupported-feature diagnostics. The target package APIs are defined in [Compiler MIR and Backend Packages](compiler-mir-backend-packages.md). | Pheidon | `make stage1-mir-backend-boundary` and `make stage1-direct-native-runtime-abi-test` | High: generated Rust and Cranelift are target implementations, not canonical semantics. |
| 8 | `compiler.backend.generated_rust` | `codegen.rs` | Legacy generated-Rust projection and `rustc` invocation compatibility. The package remains compatibility-only under [Compiler MIR and Backend Packages](compiler-mir-backend-packages.md). | Daedalus | `make stage1-mir-backend-boundary` and `cargo test --manifest-path stage1/Cargo.toml -p axiomc render_rust` | High: this package is legacy-only and must not be a dependency of self-hosted semantics. |
| 9 | `compiler.backend.native` | `cranelift_backend.rs`, direct-native tests | Direct MIR-to-native lowering, native ABI shims, native binary artifacts, native backend diagnostics. The direct-native package boundary is defined in [Compiler MIR and Backend Packages](compiler-mir-backend-packages.md). | Daedalus | `make stage1-mir-backend-boundary` and `cargo test --manifest-path stage1/Cargo.toml -p axiomc --test cranelift_backend` | High: Cranelift details must stay backend-local; direct-native evidence must not require generated Rust. |
...
- #939 MIR and backend contract package: expose MIR-to-target inputs without
  generated-Rust assumptions. See
...
- #940 HIR and ownership package: migrate typed declarations, capability checks,
  ownership, borrow, and property-clause behavior behind `compiler.hir`. See
  [Compiler HIR, Ownership, and Capability Package](compiler-hir-ownership-capability.md).
