...
| Full lib-suite and backend parity gate | The full `axiomc --lib --features run-native-tests` suite is triaged, environment-gated cases are separated, direct-native failures are repaired or explicitly linked to blockers, and parity evidence is green before final Rust removal. | Blocked by [#1255](https://github.com/OMT-Global/axiomlang/issues/1255). The full suite now runs green as a `CI Gate` dependency in `pr-fast-ci.yml` (683 passed / 8 ignored); the triage manifest checked by `make stage1-full-lib-triage` records the 8 serving tests `#[ignore]`d against the direct-native runtime-serving blocker [#1287](https://github.com/OMT-Global/axiomlang/issues/1287), and parity evidence remains the open criterion. | [#1255](https://github.com/OMT-Global/axiomlang/issues/1255) |
