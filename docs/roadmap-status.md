...
| [#927](https://github.com/OMT-Global/axiomlang/issues/927) Rust exit native backend parity readiness matrix | Implemented as the initial readiness gate | Close as completed when this PR lands | `docs/rust-exit-readiness.md` and `docs/rust-exit-readiness.json` define the readiness matrix and live open-blocker manifest, while `scripts/ci/check-rust-exit-readiness.sh` and `make rust-exit-readiness` emit `axiom.rust_exit.readiness.v1` JSON. The direct-native runtime ABI matrix is ready, so the gate is now expected to fail only until the remaining live blockers in `docs/rust-exit-readiness.json` are closed: #1191 for generated-Rust backend removal from the supported toolchain and #731 for the AxiOM-owned doc/LSP path. Final bootstrap issue #721 remains metadata and should not be listed as a live blocker for its own closing PR. Historical generated-Rust/default-backend issues #693 and #694 are closed and are no longer the gating tracker for removal proof. |
