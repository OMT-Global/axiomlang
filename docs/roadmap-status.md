...
| [#927](https://github.com/OMT-Global/axiomlang/issues/927) Rust exit native backend parity readiness matrix | Implemented as the initial readiness gate | Close as completed when this PR lands | `docs/rust-exit-readiness.md` and `docs/rust-exit-readiness.json` define the readiness matrix and live closure-proof manifest, while `scripts/ci/check-rust-exit-readiness.sh` and `make rust-exit-readiness` emit `axiom.rust_exit.readiness.v1` JSON. The direct-native runtime ABI matrix is ready, generated-Rust backend removal from the supported toolchain is complete under #1191, and the AxiOM-owned doc/LSP path is complete under #731. Final bootstrap issue #721 remains the governing Rust-removal gate and should not be listed as a live blocker for its own closing PR. Historical generated-Rust/default-backend issues #693 and #694 are closed and are no longer the gating tracker for removal proof. |
