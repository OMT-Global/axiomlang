...
| [#397](https://github.com/OMT-Global/axiom/issues/397) Runtime: HTTP server runtime surface | Implemented as the current stage1 HTTP server slice | Close as completed when this PR lands | `std/http.ax` exposes `serve_once(bind, body)` plus route-shaped `route(path, body)` / `serve(bind, route, max_requests)` over `http_serve_once` and `http_serve_route`; capability-denied programs fail before native execution via the shared `net` gate, and Rust integration coverage proves deterministic local request handling plus non-loopback bind rejection. |
