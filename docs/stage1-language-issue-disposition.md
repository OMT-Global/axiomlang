...
- Top-level scalar `const` declarations with compile-time evaluation have
  landed, and module-scope `static` declarations now cover scalar values,
  strings, and small tuples. `const fn`, const-sized arrays, and named constants
  in match-pattern positions remain outside the current bootstrap contract.
...
| [#222](https://github.com/OMT-Global/axiom/issues/222) Const / static evaluation | Keep open as partially landed | Top-level scalar `const` evaluation has landed, including local and imported public constants. Module-scope `static` now covers scalar values, strings, and small tuples. The issue acceptance also requires const-sized arrays and match arms, and the scope includes `const fn`, so the current evidence is partial only. |
