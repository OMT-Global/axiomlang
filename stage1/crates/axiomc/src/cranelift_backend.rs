...
struct I64StaticBindings {
    ...
    formatted_strings: HashMap<String, I64FormattedString>,
...
#[derive(Clone)]
enum I64FormattedString {
    Int(CraneliftI64Expr),
    Bool(CraneliftI64Condition),
    JsonStringifiedInt(CraneliftI64Expr),
    JsonStringifiedBool(CraneliftI64Condition),
    JsonFieldInt {
        prefix: String,
...
fn lower_i64_exit_program(program: &Program, fs_root: &Path) -> Option<I64ExitProgram> {
    ...
    static_bindings.json_field_int_wrappers = program
        .functions
        .iter()
        .filter(|function| is_i64_std_json_wrapper(function, "field_int"))
        .flat_map(|function| [function.name.clone(), function.source_name.clone()])
        .collect();
    static_bindings.json_field_bool_wrappers = program
        .functions
        .iter()
        .filter(|function| is_i64_std_json_wrapper(function, "field_bool"))
        .flat_map(|function| [function.name.clone(), function.source_name.clone()])
        .collect();
...
fn lower_i64_print_stmts(
    ...
    if let Some((_, stmts)) = lower_i64_log_field_list_len_and_write_stmts(
        OutputStream::Stdout,
        expr,
        local_indexes,
        local_conditions,
    ...
    Some(i64_formatted_string_write_stmts(
        OutputStream::Stdout,
        formatted,
        true,
    ))
...
fn lower_i64_formatted_string_write_stmt(
    ...
    Some(i64_formatted_string_write_stmt(stream, formatted))
}

fn i64_formatted_string_write_stmt(
    stream: OutputStream,
...
fn lower_i64_formatted_string_expr(
    message: &Expr,
    local_indexes: &HashMap<String, usize>,
    local_conditions: &HashMap<String, CraneliftI64Condition>,
    helper_signatures: &HashMap<&str, I64HelperSignature>,
    ...
    match message {
        Expr::VarRef {
            name,
            ty: Type::String | Type::Str,
        } => static_bindings.formatted_strings.get(name).cloned(),
        ...
        Expr::StringBorrow { expr, .. } => lower_i64_formatted_string_expr(
            expr,
            local_indexes,
            local_conditions,
            helper_signatures,
                ...
                || is_i64_log_field_int_name(name, static_bindings) =>
        {
            ...
            Some(I64FormattedString::JsonFieldInt {
                prefix,
                ...
                || is_i64_log_field_bool_name(name, static_bindings) =>
        {
            ...
            Some(I64FormattedString::JsonFieldBool {
                prefix,
                cond: lower_i64_condition(
                    value,
                    local_indexes,
...
fn i64_formatted_string_written_len(formatted: I64FormattedString) -> CraneliftI64Expr {
    match formatted {
        I64FormattedString::Int(value) => CraneliftI64Expr::Binary {
        ...
        I64FormattedString::JsonFieldInt { prefix, value } => CraneliftI64Expr::Binary {
            op: CraneliftI64BinaryOp::Add,
        ...
        I64FormattedString::JsonFieldBool { prefix, cond } => CraneliftI64Expr::Select {
            ...
            else_result: Box::new(CraneliftI64Expr::Literal(prefix.len() as i64 + 6)),
        },
        I64FormattedString::LogEvent {
            prefix,
            attributes,
...
fn i64_formatted_string_len_expr(formatted: I64FormattedString) -> CraneliftI64Expr {
    match formatted {
        ...
        I64FormattedString::JsonFieldBool { prefix, cond } => CraneliftI64Expr::Select {
            cond: Box::new(cond),
            then_result: Box::new(CraneliftI64Expr::Literal(prefix.len() as i64 + 4)),
            else_result: Box::new(CraneliftI64Expr::Literal(prefix.len() as i64 + 5)),
        },
        I64FormattedString::LogEvent {
            prefix,
            attributes,
...
fn i64_formatted_fields_len_expr(fields: &[I64FormattedString]) -> CraneliftI64Expr {
    let mut lengths = fields.iter().cloned().map(i64_formatted_string_len_expr);
    let Some(first) = lengths.next() else {
        return CraneliftI64Expr::Literal(0);
    };
...
fn i64_formatted_field_list_len_expr(
    ...
    let fields = fields
        ...
        .collect::<Option<Vec<_>>>()?;
    Some(i64_formatted_fields_len_expr(&fields))
}
...
fn lower_i64_runtime_string_len_let_stmts(
    ...
    if let Some(formatted) = lower_i64_formatted_string_expr(
        expr,
        local_indexes,
        local_conditions,
        helper_signatures,
        ...
        static_bindings
            .formatted_strings
            .insert(name.to_string(), formatted);
    }
    Some(assigns)
}
...
fn lower_i64_eprintln_message(
    ...
    Some((
        i64_formatted_string_written_len(formatted.clone()),
        i64_formatted_string_write_stmts(OutputStream::Stderr, formatted, true),
    ))
}
...
fn lower_i64_log_field_list_len_and_write_stmts(
    ...
    let fields = lower_i64_log_field_list_fields(
        message,
        local_indexes,
        local_conditions,
        helper_signatures,
    ...
    let written = i64_formatted_fields_len_expr(&fields);
...
fn lower_i64_log_field_list_fields(
    message: &Expr,
    local_indexes: &HashMap<String, usize>,
    local_conditions: &HashMap<String, CraneliftI64Condition>,
    helper_signatures: &HashMap<&str, I64HelperSignature>,
    static_bindings: &I64StaticBindings,
) -> Option<Vec<I64FormattedString>> {
...
fn lower_i64_panic_report_stmts(
    ...
    if let Some(formatted_message) = lower_i64_log_message_formatted_string_expr(
        ...
        return Some(i64_formatted_string_write_stmts(
            OutputStream::Stderr,
            report,
            true,
        ));
...
fn is_i64_json_field_int_name(name: &str, static_bindings: &I64StaticBindings) -> bool {
    static_bindings.json_field_int_wrappers.contains(name)
}

fn is_i64_json_field_bool_name(name: &str, static_bindings: &I64StaticBindings) -> bool {
    static_bindings.json_field_bool_wrappers.contains(name)
}
...
fn lower_i64_string_len_expr(
    ...
    if let Some(formatted) = lower_i64_formatted_string_expr(
        ...
        return Some(i64_formatted_string_len_expr(formatted));
    }
    match expr {
        Expr::BinaryAdd {
            op: ArithmeticOp::Add,
