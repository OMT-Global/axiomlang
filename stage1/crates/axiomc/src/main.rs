...
enum Command {
    ...
    /// Inspect or remove package-level build cache artifacts (all workspace members).
    Cache {
        path: PathBuf,
        /// Emit an axiom.stage1.v1 JSON envelope for agent/tool consumption.
        #[arg(long)]
        json: bool,
        /// Remove inventoried metadata, generated source, and Cranelift objects; preserve test caches.
        #[arg(long)]
        clean: bool,
    },
...
fn main() {
    ...
    let code = match cli.command {
        ...
        Command::Doctor { path, json } => {
            let project = path.unwrap_or_else(|| PathBuf::from("."));
            let report = doctor_report(&project, Cli::command().get_subcommands().count());
            if json {
