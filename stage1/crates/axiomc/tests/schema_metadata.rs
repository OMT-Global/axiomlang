...
fn io_reactor_schema_enforces_evidence_promotion_boundaries() {
    ...
    validator.validate(&snapshot).expect("checked I/O Reactor snapshot matches its schema");

    let mut incomplete_promotion = snapshot.clone();
    incomplete_promotion["implementation"]["tier"] = serde_json::json!("runtime_complete");
    assert!(!validator.is_valid(&incomplete_promotion), "runtime_complete requires complete executable evidence");

    let mut unavailable_without_reason = snapshot.clone();
    unavailable_without_reason["adapters"]["targets"][0].as_object_mut().expect("adapter row").remove("reason");
    assert!(!validator.is_valid(&unavailable_without_reason), "unavailable adapters require a reason");
    ...
    for adapter in complete_promotion["adapters"]["targets"].as_array_mut().expect("adapter targets") {
    ...
    validator.validate(&complete_promotion).expect("fully evidenced runtime_complete contract is promotion-capable");
}
...
fn sqlite_v1_schema_validates_the_bounded_contract() {
    ...
    let schema: Value = serde_json::from_str(
        &fs::read_to_string(compiler_contracts.join("schemas").join("axiom.sqlite-v1.schema.json"))
    ...
    assert_eq!(schema["$id"], "https://omt-global.github.io/axiom/schemas/axiom.sqlite-v1.schema.json");
    assert_eq!(schema["additionalProperties"], false);
    let validator = compile_validator(&schema);
    validator.validate(&contract).expect("checked SQLite v1 contract matches its published schema");
    for (name, mut invalid) in [
        ...
        match name {
            ...
            "runtime qualification overclaim" => invalid["scope"]["qualification"] = serde_json::json!("runtime_complete"),
            "wrong migration dialect" => invalid["migrations"]["dialect"] = serde_json::json!("postgresql"),
            "raw provider surface" => invalid["scope"]["raw_surface"] = serde_json::json!("allowed"),
            "ambient database path" => invalid["authority"]["path_policy"] = serde_json::json!("ambient_path"),
            "URI query mode" => invalid["authority"]["uri_policy"]["query"] = serde_json::json!("allowed"),
            "weakened prepared statement policy" => invalid["statements"]["sql_source"] = serde_json::json!("concatenated_sql"),
            "automatic corruption repair" => invalid["concurrency"]["corruption"] = serde_json::json!("automatic_repair"),
            "incomplete handle validation" => invalid["lifecycle"]["validation"] = serde_json::json!(["provider", "kind", "open"]),
            "incomplete audit redaction" => invalid["audit"]["forbidden"] = serde_json::json!(["sql_text"]),
            "unknown fixture field" => invalid["fixtures"][0]["sql"] = serde_json::json!("select ?"),
            "duplicate fixture" => {
                let duplicate = invalid["fixtures"][0].clone();
                invalid["fixtures"].as_array_mut().expect("fixtures array").push(duplicate);
            }
            _ => unreachable!(),
        }
