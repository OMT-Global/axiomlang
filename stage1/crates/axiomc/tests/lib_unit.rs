...
use crate::manifest::{
    CapabilityConfig, TestKind, TestTarget, capability_descriptors, load_manifest, lockfile_path,
    render_manifest,
};
use crate::mir;
...
fn render_manifest_with_capabilities(
    ...
    if env || process {
        manifest
            .push_str("unsafe_rationale = \"test fixture uses legacy unrestricted env/process\"\n");
    }
    manifest
}
...
"#;
    ...
    assert!(
        rendered
            .contains("axiom_numeric_checked_add_i64((axiom_numeric_checked_add_i64(40, 1)), 1)")
    );
    assert!(!rendered.contains("add_one!"));
...
fn parser_lowers_generic_functions_to_monomorphized_copies() {
    ...
    assert!(rendered.contains("let label: String = identity__string(String::from(\"stage1\"));"));
...
fn parser_lowers_generic_structs_and_enums_to_monomorphized_copies() {
    ...
    assert!(
        rendered.contains("let outcome: Outcome__int__string = Outcome__int__string::OkValue(7);")
    );
}
...
fn parser_lowers_array_slices() {
    ...
    assert!(rendered.contains("let window: &[i64] = tail(axiom_slice_view(&values, None, None));"));
    assert!(
            rendered
                .contains(
...
fn parser_lowers_mutable_slice_views() {
    ...
    let rendered = render_rust(&mir);
    assert!(
        rendered.contains("let rest: &mut [i64] = axiom_slice_view_mut(values, Some(1), None);")
    );
    assert!(
        rendered
            .contains("let rest: &mut [i64] = axiom_slice_view_mut(&mut values, Some(1), None);")
    );
...
fn parser_lowers_borrowed_structs_and_enums() {
    ...
    assert!(
        rendered.contains("let window: Window<'_> = tail(axiom_slice_view(&numbers, None, None));")
    );
    assert!(
            rendered.contains(
...
fn parser_lowers_tuples_and_tuple_indexing() {
    ...
    assert!(rendered.contains("let pair: (i64, String) = (7, String::from(\"stage1 tuples\"));"));
...
fn parser_lowers_maps_and_indexing() {
    ...
    assert!(
        rendered.contains("println!(\"{}\", axiom_map_get(&scores, &String::from(\"deploy\")));")
    );
}
...
fn parser_lowers_option_and_result() {
    ...
    assert!(rendered.contains("return Option::Some(BuildInfo { label: String::from(\"ok\") });"));
    assert!(rendered.contains("return Option::None;"));
    assert!(rendered.contains("fn load(ready: bool) -> Result<BuildInfo, String> {"));
    assert!(rendered.contains("return Result::Ok(BuildInfo { label: String::from(\"built\") });"));
...
fn parser_lowers_payload_enums_and_match_bindings() {
    ...
    assert!(rendered.contains("let message: Message = Message::Text(String::from(\"ready\"));"));
}
...
fn parser_lowers_multi_payload_enums_and_match_bindings() {
    ...
    assert!(
        rendered
            .contains("let message: Message = Message::Pair(7, String::from(\"tuple payload\"));")
    );
}
...
fn build_project_keeps_private_const_array_lengths_per_module() {
    ...
    create_project(&project, Some("const-array-module-namespaces-app")).expect("create project");
    fs::write(
...
fn build_project_emits_native_binary_after_match_temporary_borrow_ends() {
    ...
    create_project(&project, Some("match-temporary-borrow-release-app")).expect("create project");
    fs::write(
            project.join("src/main.ax"),
...
fn check_project_rejects_wrapped_multi_param_borrow_returns() {
    ...
    fs::write(
        ...
        .expect("write source");
    let error = check_project(&project).expect_err("wrapped multi-param borrow return should fail");
...
fn package_visibility_rejects_cross_package_imports() {
    ...
    let error = check_project(&project).expect_err("package-visible dependency import should fail");
...
fn lockfile_render_rejects_dependency_path_traversal() {
    ...
    create_project(&project, Some("lockfile-dependency-boundary-root")).expect("create project");
...
fn dependency_std_fs_wrapper_requires_dependency_fs_capability() {
    ...
    let error = check_project(&project).expect_err("dependency std/fs wrapper should require fs");
    assert_eq!(error.kind, "capability");
    assert!(
        error
            .path
...
fn unrestricted_process_requires_unsafe_rationale() {
    ...
    let error =
        load_manifest(&project).expect_err("unrestricted process should require unsafe rationale");
    assert_eq!(error.kind, "manifest");
    assert!(
        error
            .message
...
fn capability_sbom_preserves_unrestricted_process_unsafe_rationale() {
    ...
    let process_unsafe = payload["unsafe"]
        ...
        .iter()
        .find(|entry| entry["capability"] == "process" && entry["kind"] == "unsafe_unrestricted")
...
fn check_project_rejects_shadowed_const_process_command_with_manifest_allowlist() {
    ...
    create_project(&project, Some("process-shadowed-const-denied-app")).expect("create project");
    fs::write(
        project.join("axiom.toml"),
        r#"[package]
...
"#,
    ...
    let error = check_project(&project).expect_err("shadowed const process command should fail");
    assert_eq!(error.kind, "capability");
    assert!(
        error
            .message
...
"#,
    ...
    assert!(
        error
            .message
            .contains("requires a string literal when [capabilities].net hosts are unrestricted"),
        "unexpected diagnostic: {error:?}",
    );
}
...
"#,
    ...
    assert!(
        error
            .message
            .contains("requires an integer literal when [capabilities].net ports are unrestricted"),
        "unexpected diagnostic: {error:?}",
    );
}
...
fn check_project_rejects_dynamic_http_url_with_unrestricted_net() {
    ...
    create_project(&project, Some("http-url-dynamic-unrestricted-app")).expect("create project");
    fs::write(
        project.join("axiom.toml"),
        r#"[package]
...
"#,
    ...
    let error = check_project(&project).expect_err("dynamic stdlib HTTP URL should fail closed");
    assert_eq!(error.kind, "capability");
    assert!(
        error.message.contains("requires a static URL literal"),
        "unexpected diagnostic: {error:?}",
...
fn check_project_rejects_http_url_port_missing_from_manifest_allowlist() {
    ...
    create_project(&project, Some("http-url-port-not-allowlisted-app")).expect("create project");
    fs::write(
        project.join("axiom.toml"),
        r#"[package]
...
fn stage1_project_imports_synthetic_stdlib_fs_module() {
    ...
    fs::write(
        project.join("axiom.toml"),
        render_manifest_with_capabilities("stdlib-fs-app", true, false, false, false, false, false),
    )
    .expect("write manifest");
    let manifest = load_manifest(&project).expect("load manifest");
    fs::write(
...
fn closure_rejects_borrowed_slice_return_fn_value() {
    ...
    let err =
        check_project(&project).expect_err("fn closures must reject borrowed slice return values");
...
fn manifest_test_expected_error_fails_on_diagnostic_mismatch() {
    ...
    create_project(&project, Some("manifest-expected-error-mismatch-app")).expect("create project");
    fs::write(
        project.join("src/broken_test.ax"),
        "let x: int = \"not an int\"\n",
    )
...
fn std_doc_source_file_backs_virtual_module() {
    ...
    assert!(
        virtual_source.contains("pub fn render_source_markdown(file: &str, source: &str): string")
    );
    assert!(virtual_source.contains("pub fn render_source_html(file: &str, source: &str): string"));
...
fn ownership_compile_fail_corpus_reports_stable_codes() {
    ...
    for (case, kind, code, message) in cases {
        ...
        let error =
            check_project(&project).expect_err(&format!("ownership fixture {case} should fail"));
...
fn conformance_corpus_reports_stable_results() {
    ...
    assert_eq!(output.cases.len(), 147);
    assert_eq!(output.passed, 147);
...
fn check_project_infers_generic_option_helper_from_argument_type() {
    ...
    create_project(&project, Some("generic-inferred-option-helper-app")).expect("create project");
    fs::write(
        project.join("src/main.ax"),
...
fn check_project_rejects_generic_wrapper_instantiation_type_mismatch() {
    ...
    create_project(&project, Some("generic-wrapper-type-mismatch-app")).expect("create project");
    fs::write(
            project.join("src/main.ax"),
...
fn build_project_rejects_mismatched_const_sized_array_literals() {
    ...
    create_project(&project, Some("mismatched-const-sized-arrays-app")).expect("create project");
    fs::write(
        project.join("src/main.ax"),
...
fn build_project_rejects_unsized_array_binding_to_sized_array() {
    ...
    create_project(&project, Some("unsized-to-sized-array-binding-app")).expect("create project");
    fs::write(
        project.join("src/main.ax"),
        "let three: [int] = [1, 2, 3]
let two: [int; 2] = three
...
fn build_project_rejects_unsized_array_argument_to_sized_parameter() {
    ...
    create_project(&project, Some("unsized-to-sized-array-argument-app")).expect("create project");
    fs::write(
        project.join("src/main.ax"),
...
fn build_project_resolves_const_sized_arrays_inside_function_bodies() {
    ...
    fs::write(
        ...
        .expect("write source");
    let built =
        build_project(&project).expect("build project with const-sized array inside function body");
    let output = compiled_binary_command(&built.binary)
        .output()
        .expect("run compiled binary");
    assert_eq!(
...
fn check_project_rejects_mutually_recursive_struct_enum_without_indirection() {
    ...
    create_project(&project, Some("mutually-recursive-struct-enum-app")).expect("create project");
    fs::write(
        project.join("src/main.ax"),
        "struct ExprNode {\nexpr: Expr\n}\n\nenum Expr {\nWrap(ExprNode)\nLit(int)\n}\n\nprint 0\n",
    )
...
fn check_project_allows_non_copy_struct_field_move_through_call_then_sibling_use() {
    ...
    create_project(&project, Some("struct-call-partial-move-sibling-app")).expect("create project");
    fs::write(
            project.join("src/main.ax"),
...
fn check_project_rejects_whole_struct_use_after_field_move() {
    ...
    create_project(&project, Some("struct-partial-move-whole-use-app")).expect("create project");
    fs::write(
            project.join("src/main.ax"),
...
fn check_project_rejects_reusing_moved_struct_field() {
    ...
    create_project(&project, Some("struct-partial-move-field-reuse-app")).expect("create project");
    fs::write(
            project.join("src/main.ax"),
            "struct BuildInfo {\nlabel: string\nsummary: string\ncount: int\n}\n\nlet info: BuildInfo = BuildInfo { label: \"deploy\", summary: \"ready\", count: 7 }\nprint info.label\nprint info.label\n",
        )
...
fn check_project_rejects_mixed_array_literal_types() {
    ...
    .expect("write source");
    let error = check_project(&project).expect_err("array literal should require matching types");
    assert!(
        error
            .message
...
#[test]
fn check_project_accepts_array_call_slice_base() {
    let dir = tempdir().expect("tempdir");
    let project = dir.path().join("array-call-slice-base");
    create_project(&project, Some("array-call-slice-base-app")).expect("create project");
    fs::write(
            project.join("src/main.ax"),
            "fn values(): [int; 3] {\nreturn [1, 2, 3]\n}\nlet tail: &[int] = values()[1:]\nprint len(tail)\n",
        )
        .expect("write source");
    check_project(&project).expect("array-returning call should be a valid slice base");
}
...
fn check_project_rejects_aggregate_array_call_slice_base() {
    ...
    create_project(&project, Some("aggregate-array-call-slice-base-app")).expect("create project");
    fs::write(
            project.join("src/main.ax"),
...
fn check_project_rejects_slice_return_without_borrowed_param() {
    ...
    .expect("write source");
    let error = check_project(&project).expect_err("slice returns should require a borrowed param");
    assert!(
        error
            .message
...
fn check_project_rejects_ambiguous_multi_param_borrow_return() {
    ...
    create_project(&project, Some("slice-return-ambiguous-origin-app")).expect("create project");
    fs::write(
            project.join("src/main.ax"),
            "fn pick(a: &[int], b: &[int], use_a: bool): &[int] {\nif use_a {\nreturn a[0:1]\n}\nreturn b[0:1]\n}\n\nlet a: [int] = [1]\nlet b: [int] = [2]\nprint first(pick(a[:], b[:], true))\n",
        )
        .expect("write source");
    let error = check_project(&project).expect_err("elided multi-param borrow return should fail");
    assert!(error.message.contains(
            "cannot infer which parameter the returned borrow originates from; this case will require an explicit annotation when origin syntax lands"
        ));
    assert_eq!(
...
fn borrow_return_classification_infers_single_borrowed_param_origin() {
    ...
    let inferred = borrowck::classify_borrow_return(&params, &return_ty, &structs, &enums, 1, 1)
...
fn check_project_rejects_wrapped_borrow_return_without_borrowed_params() {
    ...
    create_project(&project, Some("wrapped-borrow-return-no-param-app")).expect("create project");
    fs::write(
        project.join("src/main.ax"),
        "fn choose(values: [int]): Option<&[int]> {\nreturn Some(values[1:])\n}\n\nprint 0\n",
    )
...
fn check_project_rejects_moving_owner_in_later_call_arg_after_temporary_borrow_arg() {
    ...
    create_project(&project, Some("call-arg-temporary-borrow-move-app")).expect("create project");
    fs::write(
            project.join("src/main.ax"),
...
fn check_project_rejects_borrowing_owner_in_later_call_arg_after_move_arg() {
    ...
    fs::write(
        ...
        .expect("write source");
    let error = check_project(&project)
        .expect_err("moving the owner first should still reject borrowing it later in the call");
    assert!(error.message.contains("use of moved value"));
    assert_eq!(error.kind, "ownership");
}
...
fn check_project_rejects_moving_owned_value_while_mutable_local_borrow_is_live() {
    ...
    fs::write(
        ...
        .expect("write source");
    let error = check_project(&project).expect_err("moving a mutably borrowed value should fail");
    assert!(
        error
            .message
...
fn check_project_rejects_moving_owner_during_mutable_slice_call_argument() {
    ...
    create_project(&project, Some("mut-slice-param-call-owner-move-app")).expect("create project");
    fs::write(
            project.join("src/main.ax"),
...
fn check_project_rejects_moving_whole_value_while_projection_mutably_borrowed() {
    ...
    create_project(&project, Some("mut-projection-borrow-whole-move-app")).expect("create project");
    fs::write(
            project.join("src/main.ax"),
...
fn build_project_accepts_mutable_slice_borrow_from_match_payload() {
    ...
    create_project(&project, Some("mut-match-payload-slice-borrow-app")).expect("create project");
    fs::write(
            project.join("src/main.ax"),
...
fn check_project_rejects_moving_owned_array_while_mutable_slice_borrow_is_live() {
    ...
    fs::write(
        ...
        .expect("write source");
    let error = check_project(&project).expect_err("moving a mutably borrowed owner should fail");
    assert!(
        error
            .message
            .contains("cannot move value \"values\" while borrowed slices are still live")
...
fn check_project_rejects_moving_owner_while_tuple_wrapped_mut_slice_is_live() {
    ...
    create_project(&project, Some("tuple-wrapped-live-mut-borrow-app")).expect("create project");
    fs::write(
            project.join("src/main.ax"),
        ...
        .expect("write source");
    let error =
        check_project(&project).expect_err("tuple-wrapped mutable borrow should block owner move");
    assert!(
        error
            .message
            .contains("cannot move value \"values\" while borrowed slices are still live")
...
fn check_project_rejects_moving_owner_while_option_wrapped_mut_slice_is_live() {
    ...
    create_project(&project, Some("option-wrapped-live-mut-borrow-app")).expect("create project");
    fs::write(
            project.join("src/main.ax"),
        ...
        .expect("write source");
    let error =
        check_project(&project).expect_err("option-wrapped mutable borrow should block owner move");
    assert!(
        error
            .message
            .contains("cannot move value \"values\" while borrowed slices are still live")
...
fn check_project_rejects_moving_owner_while_struct_wrapped_mut_slice_is_live() {
    ...
    create_project(&project, Some("struct-wrapped-live-mut-borrow-app")).expect("create project");
    fs::write(
            project.join("src/main.ax"),
        ...
        .expect("write source");
    let error =
        check_project(&project).expect_err("struct-wrapped mutable borrow should block owner move");
    assert!(
        error
            .message
            .contains("cannot move value \"values\" while borrowed slices are still live")
...
fn check_project_rejects_moving_owner_while_enum_wrapped_mut_slice_is_live() {
    ...
    fs::write(
        ...
        .expect("write source");
    let error =
        check_project(&project).expect_err("enum-wrapped mutable borrow should block owner move");
    assert!(
        error
            .message
            .contains("cannot move value \"values\" while borrowed slices are still live")
...
fn check_project_rejects_moving_owner_while_tuple_wrapped_slice_is_live() {
    ...
    fs::write(
        ...
        .expect("write source");
    let error = check_project(&project).expect_err("tuple-wrapped borrow should block owner move");
    assert!(
        error
            .message
            .contains("cannot move value \"values\" while borrowed slices are still live")
...
fn check_project_rejects_moving_owner_while_option_wrapped_slice_is_live() {
    ...
    fs::write(
        ...
        .expect("write source");
    let error = check_project(&project).expect_err("option-wrapped borrow should block owner move");
    assert!(
        error
            .message
            .contains("cannot move value \"values\" while borrowed slices are still live")
...
fn check_project_rejects_moving_owner_while_struct_wrapped_slice_is_live() {
    ...
    fs::write(
        ...
        .expect("write source");
    let error = check_project(&project).expect_err("struct-wrapped borrow should block owner move");
    assert!(
        error
            .message
            .contains("cannot move value \"values\" while borrowed slices are still live")
...
fn check_project_rejects_moving_owner_while_enum_wrapped_slice_is_live() {
    ...
    fs::write(
        ...
        .expect("write source");
    let error = check_project(&project).expect_err("enum-wrapped borrow should block owner move");
    assert!(
        error
            .message
            .contains("cannot move value \"values\" while borrowed slices are still live")
...
fn check_project_rejects_mixed_map_literal_key_types() {
    ...
    .expect("write source");
    let error = check_project(&project).expect_err("map literal should require matching key types");
    assert!(
        error
            .message
...
fn check_project_rejects_unsupported_map_key_type() {
    ...
    .expect("write source");
    let error = check_project(&project).expect_err("map type should reject unsupported key type");
    assert!(
        error
            .message
...
fn check_project_rejects_wrong_map_key_type_on_index() {
    ...
    .expect("write source");
    let error = check_project(&project).expect_err("map index should require matching key type");
    assert!(
        error
            .message
...
fn check_project_rejects_lowercase_const_match_pattern() {
    ...
    create_project(&project, Some("lowercase-const-match-pattern-app")).expect("create project");
    fs::write(
        project.join("src/main.ax"),
        "let status: int = 7\nmatch status {\nread_y {\nprint \"bad\"\n}\n}\n",
    )
...
fn check_project_rejects_multi_payload_match_binding_count() {
    ...
    fs::write(
        ...
        .expect("write source");
    let error = check_project(&project).expect_err("match should enforce payload binding count");
...
fn check_project_rejects_named_payload_constructor_with_positional_args() {
    ...
    fs::write(
        ...
        .expect("write source");
    let error =
        check_project(&project).expect_err("named payload variant should reject positional args");
...
fn check_project_rejects_multi_payload_constructor_arity() {
    ...
    create_project(&project, Some("multi-payload-constructor-arity-app")).expect("create project");
    fs::write(
            project.join("src/main.ax"),
...
fn check_project_rejects_named_payload_match_with_positional_bindings() {
    ...
    create_project(&project, Some("named-payload-match-positional-app")).expect("create project");
    fs::write(
            project.join("src/main.ax"),
            "enum Message {\nJob { id: int, label: string }\n}\n\nfn render(message: Message): string {\nmatch message {\nJob(id, label) {\nreturn label\n}\n}\n}\n\nprint \"skip\"\n",
        )
...
fn build_project_debug_mode_emits_source_markers_and_uses_separate_cache_key() {
    ...
    let debug_manifest = PathBuf::from(debug.debug_manifest.as_ref().expect("debug manifest path"));
    assert!(debug_manifest.exists());
    assert_eq!(debug.cache_hits, 0);
    assert_eq!(debug.cache_misses, 1);

    let generated = fs::read_to_string(generated_rust_path(&debug)).expect("read generated rust");
    let source = project
        .join("src/main.ax")
        .canonicalize()
    ...
    let manifest: serde_json::Value =
        serde_json::from_str(&fs::read_to_string(&debug_manifest).expect("read debug manifest"))
...
fn build_project_reuses_incremental_cache_until_module_changes() {
    ...
    let generated = fs::read_to_string(generated_rust_path(&first)).expect("read generated rust");
    ...
    let repaired_binary = build_project_with_options(&project, &options).expect("repair binary");
...
fn cranelift_build_reuses_cache_without_generated_rust_artifact() {
    ...
    let first = build_project_with_options(&project, &options).expect("initial cranelift build");
    assert_eq!(first.cache_hits, 0);
    assert_eq!(first.cache_misses, 1);
    assert_eq!(first.packages[0].cache_status, BuildCacheStatus::Miss);
    ...
    let second = build_project_with_options(&project, &options).expect("cached cranelift build");
    assert_eq!(second.cache_hits, 1);
    assert_eq!(second.cache_misses, 0);
    assert_eq!(second.packages[0].cache_status, BuildCacheStatus::Hit);
...
fn json_contract_adds_machine_readable_repair_hints() {
    let source_error =
        crate::diagnostics::Diagnostic::new("parse", "let binding is missing '='").with_span(3, 8);
