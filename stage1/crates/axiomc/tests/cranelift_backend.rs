...
#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_bool_conditions_to_runtime_exit_code() {
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    }
    ...
    let temp = tempfile::tempdir().expect("tempdir");
    let project = temp.path().join("bool-conditions-main-exit");
    write_bool_conditions_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift bool conditions main build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift bool conditions main binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "");
}
...
#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_bool_branch_reassignment_to_runtime_exit_code() {
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    }
    ...
    let temp = tempfile::tempdir().expect("tempdir");
    let project = temp.path().join("bool-branch-reassignment-main-exit");
    write_bool_branch_reassignment_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift bool branch reassignment main build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift bool branch reassignment main binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "");
}
...
#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_tuple_branch_reassignment_to_runtime_exit_code() {
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    }
    ...
    let temp = tempfile::tempdir().expect("tempdir");
    let project = temp.path().join("tuple-branch-reassignment-main-exit");
    write_tuple_branch_reassignment_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift tuple branch reassignment main build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift tuple branch reassignment main binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "");
}
...
#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_array_branch_reassignment_to_runtime_exit_code() {
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    }
    ...
    let temp = tempfile::tempdir().expect("tempdir");
    let project = temp.path().join("array-branch-reassignment-main-exit");
    write_array_branch_reassignment_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift array branch reassignment main build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift array branch reassignment main binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "");
}
...
#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_slice_dynamic_branch_returns_to_runtime_exit_code() {
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    }
    ...
    let temp = tempfile::tempdir().expect("tempdir");
    let project = temp.path().join("slice-dynamic-branch-returns-main-exit");
    write_slice_dynamic_branch_returns_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift slice dynamic branch returns main build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift slice dynamic branch returns main binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "");
}
...
#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_slice_dynamic_branch_return_helper_chain_to_runtime_exit_code() {
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    }
    ...
    let project = temp
        .path()
        .join("slice-dynamic-branch-return-helper-chain-main-exit");
    write_slice_dynamic_branch_return_helper_chain_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift slice dynamic branch return helper chain main build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift slice dynamic branch return helper chain main binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "");
}
...
#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_bool_slice_dynamic_branch_return_helper_chain_to_runtime_exit_code() {
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    }
    ...
    let project = temp
        .path()
        .join("bool-slice-dynamic-branch-return-helper-chain-main-exit");
    write_bool_slice_dynamic_branch_return_helper_chain_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift bool slice dynamic branch return helper chain main build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift bool slice dynamic branch return helper chain main binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "");
}
...
fn cranelift_backend_lowers_slice_branch_return_helper_args_to_runtime_exit_code() {
    ...
    let project = temp
        .path()
        .join("slice-branch-return-helper-args-main-exit");
    write_slice_branch_return_helper_args_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift slice branch return helper args main build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift slice branch return helper args main binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "");
}
...
#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_slice_return_subranges_to_runtime_exit_code() {
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    }
    ...
    let temp = tempfile::tempdir().expect("tempdir");
    let project = temp.path().join("slice-return-subranges-main-exit");
    write_slice_return_subranges_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift slice return subranges main build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift slice return subranges main binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "");
}
...
#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_string_concat_reassignment_to_runtime_exit_code() {
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    }
    ...
    let temp = tempfile::tempdir().expect("tempdir");
    let project = temp.path().join("string-concat-reassignment-main-exit");
    write_string_concat_reassignment_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift string concat reassignment main build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift string concat reassignment main binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "");
}
...
fn cranelift_backend_lowers_string_dynamic_projection_reassignment_to_runtime_exit_code() {
    ...
    let project = temp
        .path()
        .join("string-dynamic-projection-reassignment-main-exit");
    write_string_dynamic_projection_reassignment_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift string dynamic projection reassignment main build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift string dynamic projection reassignment main binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "");
}
...
#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_string_loop_dynamic_projection_reassignment_to_runtime_exit_code() {
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    }
    ...
    let project = temp
        .path()
        .join("string-loop-dynamic-projection-reassignment-main-exit");
    write_string_loop_dynamic_projection_reassignment_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift string loop dynamic projection reassignment main build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift string loop dynamic projection reassignment main binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "");
}
...
#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_string_loop_reassignment_to_runtime_exit_code() {
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    }
    ...
    let temp = tempfile::tempdir().expect("tempdir");
    let project = temp.path().join("string-loop-reassignment-main-exit");
    write_string_loop_reassignment_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift string loop reassignment main build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift string loop reassignment main binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "");
}
...
#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_string_helper_reassignment_to_runtime_exit_code() {
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    }
    ...
    let temp = tempfile::tempdir().expect("tempdir");
    let project = temp.path().join("string-helper-reassignment-main-exit");
    write_string_helper_reassignment_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift string helper reassignment main build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift string helper reassignment main binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "");
}
...
#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_string_helper_dynamic_projection_reassignment_to_runtime_exit_code() {
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    }
    ...
    let project = temp
        .path()
        .join("string-helper-dynamic-projection-reassignment-main-exit");
    write_string_helper_dynamic_projection_reassignment_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift string helper dynamic projection reassignment main build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift string helper dynamic projection reassignment main binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "");
}
...
#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_std_regex_wrapper_reassignment_to_runtime_exit_code() {
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    }
    ...
    assert!(
        output.status.success(),
        "cranelift std regex wrapper reassignment build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift std regex wrapper reassignment binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "");
}
...
fn write_bool_conditions_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src")).expect("create bool conditions main exit src");
    fs::write(
        project.join("axiom.toml"),
        "[package]\nname = \"cranelift-bool-conditions-main-exit\"\nversion = \"0.1.0\"\n\n[build]\nentry = \"src/main.ax\"\nout_dir = \"dist\"\n\n[capabilities]\nfs = false\nnet = false\nprocess = false\nenv = false\nclock = false\ncrypto = false\n",
    )
    .expect("write bool conditions main exit manifest");
    fs::write(
        project.join("axiom.lock"),
        "version = 1\n\n[[package]]\nname = \"cranelift-bool-conditions-main-exit\"\nversion = \"0.1.0\"\nsource = \"path\"\n",
    )
    .expect("write bool conditions main exit lockfile");
    fs::write(
        project.join("src/main.ax"),
        "static STATIC_READY: bool = true\nstatic STATIC_BLOCKED: bool = false\n\nfn eq_gate(lhs: bool, rhs: bool): bool {\nreturn lhs == rhs\n}\n\nfn ne_gate(lhs: bool, rhs: bool): bool {\nreturn lhs != rhs\n}\n\nfn choose(flag: bool, left: bool, right: bool): bool {\nif flag {\nreturn left\n} else {\nreturn right\n}\n}\n\nfn main(): int {\nlet dynamic_ready: bool = 40 + 2 == 42\nlet dynamic_blocked: bool = 7 > 9\nlet eq_local: bool = dynamic_ready == STATIC_READY\nlet ne_local: bool = dynamic_ready != dynamic_blocked\nlet eq_helper: bool = eq_gate(dynamic_ready, true)\nlet ne_helper: bool = ne_gate(dynamic_blocked, true)\nlet chosen_true: bool = choose(eq_helper, dynamic_ready, dynamic_blocked)\nlet chosen_false: bool = choose(dynamic_blocked, dynamic_ready, STATIC_BLOCKED)\nlet composed: bool = (eq_local && ne_local && eq_helper) || STATIC_BLOCKED\nif composed && ne_helper && chosen_true && chosen_false == false {\nreturn 48\n} else {\nreturn 1\n}\n}\n",
    )
    .expect("write bool conditions main exit source");
}

fn write_bool_branch_reassignment_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src"))
        .expect("create bool branch reassignment main exit project src");
    fs::write(
        project.join("axiom.toml"),
        "[package]\nname = \"cranelift-bool-branch-reassignment-main-exit\"\nversion = \"0.1.0\"\n\n[build]\nentry = \"src/main.ax\"\nout_dir = \"dist\"\n\n[capabilities]\nfs = false\nnet = false\nprocess = false\nenv = false\nclock = false\ncrypto = false\n",
    )
    .expect("write bool branch reassignment main exit manifest");
    fs::write(
        project.join("axiom.lock"),
        "version = 1\n\n[[package]]\nname = \"cranelift-bool-branch-reassignment-main-exit\"\nversion = \"0.1.0\"\nsource = \"path\"\n",
    )
    .expect("write bool branch reassignment main exit lockfile");
    fs::write(
        project.join("src/main.ax"),
        "fn is_answer(value: int): bool {\nreturn value == 48\n}\n\nfn gate_value(left: bool, right: bool): bool {\nreturn left && right == false\n}\n\nfn main(): int {\nlet selected: bool = false\nlet fallback: bool = true\nlet gate: bool = true\nif gate {\nselected = is_answer(48)\nfallback = is_answer(1)\n} else {\nselected = false\nfallback = true\n}\nlet accepted: bool = gate_value(selected, fallback)\nif accepted && selected && fallback == false {\nreturn 48\n} else {\nreturn 1\n}\n}\n",
    )
    .expect("write bool branch reassignment main exit source");
}
...
fn write_tuple_branch_reassignment_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src"))
        .expect("create tuple branch reassignment main exit project src");
    fs::write(
        project.join("axiom.toml"),
        "[package]\nname = \"cranelift-tuple-branch-reassignment-main-exit\"\nversion = \"0.1.0\"\n\n[build]\nentry = \"src/main.ax\"\nout_dir = \"dist\"\n\n[capabilities]\nfs = false\nnet = false\nprocess = false\nenv = false\nclock = false\ncrypto = false\n",
    )
    .expect("write tuple branch reassignment main exit manifest");
    fs::write(
        project.join("axiom.lock"),
        "version = 1\n\n[[package]]\nname = \"cranelift-tuple-branch-reassignment-main-exit\"\nversion = \"0.1.0\"\nsource = \"path\"\n",
    )
    .expect("write tuple branch reassignment main exit lockfile");
    fs::write(
        project.join("src/main.ax"),
        "fn make_pair(value: int, enabled: bool): (int, bool) {\nreturn (value, enabled)\n}\n\nfn score(pair: (int, bool)): int {\nif pair.1 {\nreturn pair.0\n} else {\nreturn 1\n}\n}\n\nfn main(): int {\nlet selected: (int, bool) = (0, false)\nlet fallback: (int, bool) = (0, false)\nlet gate: bool = true\nif gate {\nselected = make_pair(48, true)\nfallback = make_pair(49, false)\n} else {\nselected = (1, false)\nfallback = (1, false)\n}\nlet selected_code: int = score(selected)\nlet fallback_code: int = score(fallback)\nif selected.1 && selected_code == 48 && fallback_code == 1 {\nreturn selected_code\n} else {\nreturn fallback_code\n}\n}\n",
    )
    .expect("write tuple branch reassignment main exit source");
}
...
fn write_array_branch_reassignment_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src"))
        .expect("create array branch reassignment main exit project src");
    fs::write(
        project.join("axiom.toml"),
        "[package]\nname = \"cranelift-array-branch-reassignment-main-exit\"\nversion = \"0.1.0\"\n\n[build]\nentry = \"src/main.ax\"\nout_dir = \"dist\"\n\n[capabilities]\nfs = false\nnet = false\nprocess = false\nenv = false\nclock = false\ncrypto = false\n",
    )
    .expect("write array branch reassignment main exit manifest");
    fs::write(
        project.join("axiom.lock"),
        "version = 1\n\n[[package]]\nname = \"cranelift-array-branch-reassignment-main-exit\"\nversion = \"0.1.0\"\nsource = \"path\"\n",
    )
    .expect("write array branch reassignment main exit lockfile");
    fs::write(
        project.join("src/main.ax"),
        "fn make_values(left: int, right: int): [int; 2] {\nreturn [left, right]\n}\n\nfn make_flags(ready: bool): [bool; 2] {\nreturn [ready, false]\n}\n\nfn score(values: [int; 2], flags: [bool; 2]): int {\nif flags[0] && flags[1] == false {\nreturn values[0] + values[1]\n} else {\nreturn 1\n}\n}\n\nfn main(): int {\nlet selected: [int; 2] = [0, 0]\nlet fallback: [int; 2] = [0, 0]\nlet flags: [bool; 2] = [false, true]\nlet gate: bool = true\nif gate {\nselected = make_values(20, 28)\nfallback = make_values(1, 0)\nflags = make_flags(true)\n} else {\nselected = [1, 0]\nfallback = [1, 0]\nflags = make_flags(false)\n}\nlet selected_code: int = score(selected, flags)\nlet fallback_code: int = fallback[0] + fallback[1]\nif selected_code == 48 && fallback_code == 1 {\nreturn selected_code\n} else {\nreturn fallback_code\n}\n}\n",
    )
    .expect("write array branch reassignment main exit source");
}
...
fn write_slice_dynamic_branch_returns_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src"))
        .expect("create slice dynamic branch returns main exit project src");
    fs::write(
        project.join("axiom.toml"),
        "[package]\nname = \"cranelift-slice-dynamic-branch-returns-main-exit\"\nversion = \"0.1.0\"\n\n[build]\nentry = \"src/main.ax\"\nout_dir = \"dist\"\n\n[capabilities]\nfs = false\nnet = false\nprocess = false\nenv = false\nclock = false\ncrypto = false\n",
    )
    .expect("write slice dynamic branch returns main exit manifest");
    fs::write(
        project.join("axiom.lock"),
        "version = 1\n\n[[package]]\nname = \"cranelift-slice-dynamic-branch-returns-main-exit\"\nversion = \"0.1.0\"\nsource = \"path\"\n",
    )
    .expect("write slice dynamic branch returns main exit lockfile");
    fs::write(
        project.join("src/main.ax"),
...
fn write_slice_dynamic_branch_return_helper_chain_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src"))
        .expect("create slice dynamic branch return helper chain main exit project src");
    fs::write(
        project.join("axiom.toml"),
        "[package]\nname = \"cranelift-slice-dynamic-branch-return-helper-chain-main-exit\"\nversion = \"0.1.0\"\n\n[build]\nentry = \"src/main.ax\"\nout_dir = \"dist\"\n\n[capabilities]\nfs = false\nnet = false\nprocess = false\nenv = false\nclock = false\ncrypto = false\n",
    )
    .expect("write slice dynamic branch return helper chain main exit manifest");
    fs::write(
        project.join("axiom.lock"),
        "version = 1\n\n[[package]]\nname = \"cranelift-slice-dynamic-branch-return-helper-chain-main-exit\"\nversion = \"0.1.0\"\nsource = \"path\"\n",
    )
    .expect("write slice dynamic branch return helper chain main exit lockfile");
    fs::write(
        project.join("src/main.ax"),
        "fn choose_values(flag: bool, values: &[int]): &[int] {\nif flag {\nreturn values[0:3]\n} else {\nreturn values[1:4]\n}\n}\n\nfn identity(values: &[int]): &[int] {\nreturn values\n}\n\nfn score(values: &[int]): int {\nreturn len(values) + first(values) + last(values)\n}\n\nfn main(): int {\nlet input: [int; 4] = [9, 20, 26, 9]\nlet values: &[int] = input[:]\nlet flags_input: [bool; 4] = [false, true, false, true]\nlet flags: &[bool] = flags_input[:]\nlet pick_index: int = 1\nlet selector: bool = first(flags) == false\nlet selected_values: &[int] = choose_values(selector, values)\nlet selected_window: &[int] = selected_values[1:3]\nlet forwarded: &[int] = identity(selected_window)\nlet selected_code: int = score(forwarded)\nlet selected_pick: int = forwarded[pick_index] + 22\nif selector && selected_code == 48 && selected_pick == 48 {\nreturn 48\n} else {\nreturn 1\n}\n}\n",
    )
    .expect("write slice dynamic branch return helper chain main exit source");
}

fn write_bool_slice_dynamic_branch_return_helper_chain_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src"))
        .expect("create bool slice dynamic branch return helper chain main exit project src");
    fs::write(
        project.join("axiom.toml"),
        "[package]\nname = \"cranelift-bool-slice-dynamic-branch-return-helper-chain-main-exit\"\nversion = \"0.1.0\"\n\n[build]\nentry = \"src/main.ax\"\nout_dir = \"dist\"\n\n[capabilities]\nfs = false\nnet = false\nprocess = false\nenv = false\nclock = false\ncrypto = false\n",
    )
    .expect("write bool slice dynamic branch return helper chain main exit manifest");
    fs::write(
        project.join("axiom.lock"),
        "version = 1\n\n[[package]]\nname = \"cranelift-bool-slice-dynamic-branch-return-helper-chain-main-exit\"\nversion = \"0.1.0\"\nsource = \"path\"\n",
    )
    .expect("write bool slice dynamic branch return helper chain main exit lockfile");
    fs::write(
        project.join("src/main.ax"),
        "fn choose_flags(flag: bool, flags: &[bool]): &[bool] {\nif flag {\nreturn flags[0:3]\n} else {\nreturn flags[1:4]\n}\n}\n\nfn identity_flags(flags: &[bool]): &[bool] {\nreturn flags\n}\n\nfn gate(flags: &[bool], index: int): bool {\nreturn first(flags) == false && last(flags) && flags[index]\n}\n\nfn main(): int {\nlet flags_input: [bool; 4] = [false, true, false, true]\nlet flags: &[bool] = flags_input[:]\nlet pick_index: int = 1\nlet selector: bool = first(flags) == false\nlet selected_flags: &[bool] = choose_flags(selector, flags)\nlet selected_window: &[bool] = selected_flags[0:2]\nlet forwarded: &[bool] = identity_flags(selected_window)\nlet selected_gate: bool = gate(forwarded, pick_index)\nif selector && selected_gate {\nreturn 48\n} else {\nreturn 1\n}\n}\n",
    )
    .expect("write bool slice dynamic branch return helper chain main exit source");
}

fn write_slice_branch_return_helper_args_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src"))
        .expect("create slice branch return helper args main exit project src");
    fs::write(
        project.join("axiom.toml"),
        "[package]\nname = \"cranelift-slice-branch-return-helper-args-main-exit\"\nversion = \"0.1.0\"\n\n[build]\nentry = \"src/main.ax\"\nout_dir = \"dist\"\n\n[capabilities]\nfs = false\nnet = false\nprocess = false\nenv = false\nclock = false\ncrypto = false\n",
    )
    .expect("write slice branch return helper args main exit manifest");
    fs::write(
        project.join("axiom.lock"),
        "version = 1\n\n[[package]]\nname = \"cranelift-slice-branch-return-helper-args-main-exit\"\nversion = \"0.1.0\"\nsource = \"path\"\n",
    )
    .expect("write slice branch return helper args main exit lockfile");
    fs::write(
        project.join("src/main.ax"),
        "fn choose_values(flag: bool, values: &[int]): &[int] {\nif flag {\nreturn values[0:2]\n} else {\nreturn values[1:3]\n}\n}\n\nfn choose_flags(flag: bool, flags: &[bool]): &[bool] {\nif flag {\nlet chosen: &[bool] = flags[0:2]\nreturn chosen\n} else {\nlet backup: &[bool] = flags[1:3]\nreturn backup\n}\n}\n\nfn score(values: &[int]): int {\nreturn len(values) + first(values) + last(values)\n}\n\nfn gate(flags: &[bool], index: int): bool {\nreturn first(flags) == false && last(flags) && flags[index]\n}\n\nfn main(): int {\nlet input: [int; 3] = [20, 26, 20]\nlet values: &[int] = input[:]\nlet flags_input: [bool; 3] = [false, true, false]\nlet flags: &[bool] = flags_input[:]\nlet pick_index: int = 1\nlet selected_values: &[int] = choose_values(true, values)\nlet fallback_values: &[int] = choose_values(false, values)\nlet selected_flags: &[bool] = choose_flags(true, flags)\nlet fallback_flags: &[bool] = choose_flags(false, flags)\nlet selected_code: int = score(selected_values)\nlet fallback_code: int = score(fallback_values)\nlet selected_gate: bool = gate(selected_flags, pick_index)\nlet fallback_gate: bool = first(fallback_flags) && last(fallback_flags) == false\nif selected_gate && fallback_gate && selected_code == 48 && fallback_code == 48 {\nreturn 48\n} else {\nreturn 1\n}\n}\n",
    )
    .expect("write slice branch return helper args main exit source");
}

fn write_slice_return_subranges_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src"))
        .expect("create slice return subranges main exit project src");
    fs::write(
        project.join("axiom.toml"),
        "[package]\nname = \"cranelift-slice-return-subranges-main-exit\"\nversion = \"0.1.0\"\n\n[build]\nentry = \"src/main.ax\"\nout_dir = \"dist\"\n\n[capabilities]\nfs = false\nnet = false\nprocess = false\nenv = false\nclock = false\ncrypto = false\n",
    )
    .expect("write slice return subranges main exit manifest");
    fs::write(
        project.join("axiom.lock"),
        "version = 1\n\n[[package]]\nname = \"cranelift-slice-return-subranges-main-exit\"\nversion = \"0.1.0\"\nsource = \"path\"\n",
    )
    .expect("write slice return subranges main exit lockfile");
    fs::write(
        project.join("src/main.ax"),
        "fn choose_values(flag: bool, values: &[int]): &[int] {\nif flag {\nreturn values[0:3]\n} else {\nreturn values[1:4]\n}\n}\n\nfn choose_flags(flag: bool, flags: &[bool]): &[bool] {\nif flag {\nlet chosen: &[bool] = flags[0:3]\nreturn chosen\n} else {\nlet backup: &[bool] = flags[1:4]\nreturn backup\n}\n}\n\nfn score(values: &[int]): int {\nreturn len(values) + first(values) + last(values)\n}\n\nfn gate(flags: &[bool], index: int): bool {\nreturn first(flags) == false && last(flags) && flags[index]\n}\n\nfn main(): int {\nlet input: [int; 4] = [9, 20, 26, 9]\nlet values: &[int] = input[:]\nlet flags_input: [bool; 4] = [true, false, true, false]\nlet flags: &[bool] = flags_input[:]\nlet pick_index: int = 1\nlet selected_values: &[int] = choose_values(true, values)\nlet selected_window: &[int] = selected_values[1:3]\nlet selected_flags: &[bool] = choose_flags(true, flags)\nlet selected_flag_window: &[bool] = selected_flags[1:3]\nlet selected_code: int = len(selected_window) + first(selected_window) + last(selected_window)\nlet selected_pick: int = selected_window[pick_index] + 22\nlet helper_code: int = score(selected_window)\nlet selected_gate: bool = gate(selected_flag_window, pick_index)\nif selected_gate && selected_code == 48 && selected_pick == 48 && helper_code == 48 {\nreturn 48\n} else {\nreturn 1\n}\n}\n",
    )
    .expect("write slice return subranges main exit source");
}
...
fn write_string_concat_reassignment_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src"))
        .expect("create string concat reassignment main exit project src");
    fs::write(
        project.join("axiom.toml"),
        r#"[package]
name = "cranelift-string-concat-reassignment-main-exit"
version = "0.1.0"
...
"#,
    ...
    .expect("write string concat reassignment main exit manifest");
    fs::write(
        project.join("axiom.lock"),
        r#"version = 1
...
"#,
    ...
    .expect("write string concat reassignment main exit lockfile");
    fs::write(
        project.join("src/main.ax"),
...
fn write_string_dynamic_projection_reassignment_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src"))
        .expect("create string dynamic projection reassignment main exit project src");
    fs::write(
        project.join("axiom.toml"),
        r#"[package]
name = "cranelift-string-dynamic-projection-reassignment-main-exit"
version = "0.1.0"
...
"#,
    ...
    .expect("write string dynamic projection reassignment main exit manifest");
    fs::write(
        project.join("axiom.lock"),
        r#"version = 1
...
"#,
    ...
    .expect("write string dynamic projection reassignment main exit lockfile");
    fs::write(
        project.join("src/main.ax"),
        r#"fn main(): int {
let parsed: int = match json_parse_int("12345") { Some(value) => value, None => 1 }
let parsed_gate: int = match json_parse_int("12345") { Some(value) => value, None => 1 }
let flag: bool = match json_parse_bool("true") { Some(value) => value, None => false }
let selected: string = "unset"
let fallback: string = "unset"
if flag {
selected = json_stringify_int(parsed)
fallback = json_stringify_bool(flag)
} else {
selected = "0"
fallback = "false"
}
let selected_len: int = len(selected)
let fallback_len: int = len(fallback)
if selected_len == 5 && fallback_len == 4 && parsed_gate == 12345 {
return 48
} else {
return 1
}
...
"#,
    ...
    .expect("write string dynamic projection reassignment main exit source");
}

fn write_string_loop_dynamic_projection_reassignment_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src"))
        .expect("create string loop dynamic projection reassignment main exit project src");
    fs::write(
        project.join("axiom.toml"),
        r#"[package]
name = "cranelift-string-loop-dynamic-projection-reassignment-main-exit"
version = "0.1.0"
...
"#,
    ...
    .expect("write string loop dynamic projection reassignment main exit manifest");
    fs::write(
        project.join("axiom.lock"),
        r#"version = 1
...
"#,
    ...
    .expect("write string loop dynamic projection reassignment main exit lockfile");
    fs::write(
        project.join("src/main.ax"),
        r#"fn main(): int {
let parsed_gate: int = match json_parse_int("12345") { Some(value) => value, None => 1 }
...
let selected_len: int = len(selected)
let fallback_len: int = len(fallback)
if selected_len == 5 && fallback_len == 4 && parsed_gate == 12345 && index == 2 {
return 48
} else {
return 1
}
...
"#,
    ...
    .expect("write string loop dynamic projection reassignment main exit source");
}

fn write_string_loop_reassignment_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src"))
        .expect("create string loop reassignment main exit project src");
    fs::write(
        project.join("axiom.toml"),
        r#"[package]
name = "cranelift-string-loop-reassignment-main-exit"
version = "0.1.0"
...
"#,
    ...
    .expect("write string loop reassignment main exit manifest");
    fs::write(
        project.join("axiom.lock"),
        r#"version = 1
...
"#,
    ...
    .expect("write string loop reassignment main exit lockfile");
    fs::write(
        project.join("src/main.ax"),
        r#"fn main(): int {
let selected: string = "unset"
...
selected = string_clone("direct-native")
fallback = string_trim("  fallback  ")
}
index = index + 1
}
...
"#,
    ...
    .expect("write string loop reassignment main exit source");
}

fn write_string_helper_reassignment_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src"))
        .expect("create string helper reassignment main exit project src");
    fs::write(
        project.join("axiom.toml"),
        r#"[package]
name = "cranelift-string-helper-reassignment-main-exit"
version = "0.1.0"
...
"#,
    ...
    .expect("write string helper reassignment main exit manifest");
    fs::write(
        project.join("axiom.lock"),
        r#"version = 1
...
"#,
    ...
    .expect("write string helper reassignment main exit lockfile");
    fs::write(
        project.join("src/main.ax"),
        r#"fn selected_score(flag: bool): int {
let selected: string = "unset"
let fallback: string = "unset"
if flag {
selected = string_clone("direct-native")
fallback = string_trim("  helper  ")
} else {
selected = "rust"
fallback = "bad"
}
...
let selected_score_true: int = selected_score(true)
let selected_score_false: int = selected_score(false)
if selected_score_true == 19 && selected_score_false == 7 {
return 48
} else {
return 1
}
...
"#,
    ...
    .expect("write string helper reassignment main exit source");
}

fn write_string_helper_dynamic_projection_reassignment_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src"))
        .expect("create string helper dynamic projection reassignment main exit project src");
    fs::write(
        project.join("axiom.toml"),
        r#"[package]
name = "cranelift-string-helper-dynamic-projection-reassignment-main-exit"
version = "0.1.0"
...
"#,
    ...
    .expect("write string helper dynamic projection reassignment main exit manifest");
    fs::write(
        project.join("axiom.lock"),
        r#"version = 1
...
"#,
    ...
    .expect("write string helper dynamic projection reassignment main exit lockfile");
    fs::write(
        project.join("src/main.ax"),
        r#"fn selected_score(flag: bool): int {
let selected: string = "unset"
...
let selected_score_true: int = selected_score(true)
let selected_score_false: int = selected_score(false)
if selected_score_true == 9 && selected_score_false == 6 {
return 48
} else {
return 1
}
...
let selected: string = "unset"
if flag {
selected = replace_all("[0-9]+", ISSUE_TEXT, "#")
} else {
selected = "bad"
}
return len(selected)
...
let branch_selected: string = "unset"
if true {
branch_selected = replace_all("[0-9]+", ISSUE_TEXT, "#")
} else {
branch_selected = "bad"
}
let loop_selected: string = "unset"
...
loop_selected = replace_all("[0-9]+", ISSUE_TEXT, "#")
}
index = index + 1
}
let branch_len: int = len(branch_selected)
...
let helper_false_len: int = helper_score(false)
let match_gate: bool = is_match("^issue-[0-9]+-ready$", ISSUE_TEXT)
if match_gate && branch_len == 13 && loop_len == 13 && helper_true_len == 13 && helper_false_len == 3 && index == 2 {
return 48
} else {
return 1
}
...
fn write_std_json_wrapper_reassignment_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src"))
        .expect("create std json wrapper reassignment project src");
    fs::write(
        project.join("axiom.toml"),
        "[package]\nname = \"cranelift-std-json-wrapper-reassignment-main-exit\"\nversion = \"0.1.0\"\n\n[build]\nentry = \"src/main.ax\"\nout_dir = \"dist\"\n\n[capabilities]\nfs = false\nnet = false\nprocess = false\nenv = false\nclock = false\ncrypto = false\n",
    )
    .expect("write std json wrapper reassignment manifest");
    fs::write(
        project.join("axiom.lock"),
        "version = 1\n\n[[package]]\nname = \"cranelift-std-json-wrapper-reassignment-main-exit\"\nversion = \"0.1.0\"\nsource = \"path\"\n",
    )
