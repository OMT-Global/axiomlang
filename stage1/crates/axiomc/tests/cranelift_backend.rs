...
#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_known_eprintln_to_native_stderr_runtime_exit_code() {
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    }
    ...
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "");
    assert_eq!(String::from_utf8_lossy(&run.stderr), "native stderr\n");
}
...
fn cranelift_backend_lowers_known_print_to_native_stdout_runtime_exit_code() {
    ...
    let temp = tempfile::tempdir().expect("tempdir");
    let project = temp.path().join("logging-stdout-main-exit");
    write_logging_stdout_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift logging stdout main build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift logging stdout main binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(
        String::from_utf8_lossy(&run.stdout),
        "native stdout\n42\ntrue\n"
...
#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_runtime_print_format_to_native_stdout_runtime_exit_code() {
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    }
    ...
    let temp = tempfile::tempdir().expect("tempdir");
    let project = temp.path().join("runtime-print-format-main-exit");
    write_runtime_print_format_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift runtime print format build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift runtime print format binary");
    assert_eq!(run.status.code(), Some(48));
...
#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_cloned_runtime_format_output_to_native_streams_runtime_exit_code() {
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    }
    ...
    let temp = tempfile::tempdir().expect("tempdir");
    let project = temp.path().join("cloned-runtime-format-output-main-exit");
    write_cloned_runtime_format_output_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift cloned runtime format output build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift cloned runtime format output binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "21\n");
    assert_eq!(String::from_utf8_lossy(&run.stderr), "true\n");
}

#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_json_quoted_runtime_format_output_to_native_streams_runtime_exit_code()
{
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    ...
    let project = temp
        .path()
        .join("json-quoted-runtime-format-output-main-exit");
    write_json_quoted_runtime_format_output_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift JSON-quoted runtime format output build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift JSON-quoted runtime format output binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "\"21\"\n");
    assert_eq!(String::from_utf8_lossy(&run.stderr), "\"true\"\n");
}
...
fn cranelift_backend_lowers_runtime_bool_print_to_native_stdout_runtime_exit_code() {
    ...
    let temp = tempfile::tempdir().expect("tempdir");
    let project = temp.path().join("runtime-bool-stdout-main-exit");
    write_runtime_bool_stdout_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift runtime bool stdout main build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift runtime bool stdout main binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "true\nfalse\n");
    assert_eq!(String::from_utf8_lossy(&run.stderr), "");
}
...
#[cfg(not(windows))]
#[test]
fn cranelift_backend_lowers_runtime_int_print_to_native_stdout_runtime_exit_code() {
    if which::which("cc").is_err() {
        eprintln!("skipping cranelift backend smoke test because cc is unavailable");
        return;
    }
    ...
    let temp = tempfile::tempdir().expect("tempdir");
    let project = temp.path().join("runtime-int-stdout-main-exit");
    write_runtime_int_stdout_main_exit_project(&project);
    ...
    assert!(
        output.status.success(),
        "cranelift runtime int stdout main build failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let run = Command::new(binary)
        .output()
        .expect("run cranelift runtime int stdout main binary");
    assert_eq!(run.status.code(), Some(48));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "21\n-21\n");
    assert_eq!(String::from_utf8_lossy(&run.stderr), "");
}
...
fn write_std_json_wrapper_main_exit_project(project: &Path) {
    ...
    fs::write(
        project.join("axiom.toml"),
        "[package]\nname = \"cranelift-std-json-wrapper-main-exit\"\nversion = \"0.1.0\"\n\n[build]\nentry = \"src/main.ax\"\nout_dir = \"dist\"\n\n[capabilities]\nfs = false\nnet = false\nprocess = false\nenv = false\nclock = false\ncrypto = false\n\n[unsafe_rationale]\nstdio = \"Direct-native stdout/stderr regression covers std/json stringify_value wrappers for issue 1001.\"\n",
    )
...
let runtime_fields2_len: int = len(fields2(field_int("runtime_attempt", make_attempt()), field_bool("runtime_ready", make_ready())))
...
fn main(): int {
let written: int = eprintln("native stderr")
if written == 14 {
return 48
} else {
return 1
...
fn write_logging_stdout_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src")).expect("create logging stdout main project src");
    fs::write(
        project.join("axiom.toml"),
        r#"[package]
name = "cranelift-logging-stdout-main-exit"
version = "0.1.0"
...
[unsafe_rationale]
stdio = "Direct-native stdout regression covers known print statements for issue 1001."
"#,
    )
    .expect("write logging stdout main manifest");
    fs::write(
        project.join("axiom.lock"),
        r#"version = 1
...
"#,
    ...
    .expect("write logging stdout main lockfile");
    fs::write(
        project.join("src/main.ax"),
        r#"fn main(): int {
print "native stdout"
...
fn write_runtime_print_format_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src")).expect("create runtime print format project src");
    fs::write(
        project.join("axiom.toml"),
        r#"[package]
name = "cranelift-runtime-print-format-main-exit"
version = "0.1.0"
...
[unsafe_rationale]
stdio = "Direct-native stdout regression covers runtime formatted print statements for issue 1001."
"#,
    )
    .expect("write runtime print format manifest");
    fs::write(
        project.join("axiom.lock"),
        r#"version = 1
...
"#,
    ...
    .expect("write runtime print format lockfile");
    fs::write(
        project.join("src/main.ax"),
        r#"import "std/json.ax"
...
print stringify_int(total)
print stringify_bool(enabled)
if total == 21 && enabled {
return 48
} else {
return 1
...
"#,
    ...
    .expect("write runtime print format source");
}

fn write_cloned_runtime_format_output_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src"))
        .expect("create cloned runtime format output project src");
    fs::write(
        project.join("axiom.toml"),
        r#"[package]
name = "cranelift-cloned-runtime-format-output-main-exit"
version = "0.1.0"
...
[unsafe_rationale]
stdio = "Direct-native stdout/stderr regression covers cloned runtime formatted output for issue 1001."
"#,
    )
    .expect("write cloned runtime format output manifest");
    fs::write(
        project.join("axiom.lock"),
        r#"version = 1
...
"#,
    ...
    .expect("write cloned runtime format output lockfile");
    fs::write(
        project.join("src/main.ax"),
        r#"import "std/io.ax"
import "std/json.ax"
...
let total_text: string = stringify_int(total)
let enabled_text: string = stringify_bool(enabled)
let total_clone: string = string_clone(total_text)
let enabled_clone: string = string_clone(enabled_text)
print total_clone
let written: int = eprintln(enabled_clone)
if total == 21 && enabled && written == 5 {
return 48
} else {
return 1
}
...
"#,
    ...
    .expect("write cloned runtime format output source");
}

fn write_json_quoted_runtime_format_output_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src"))
        .expect("create JSON-quoted runtime format output project src");
    fs::write(
        project.join("axiom.toml"),
        r#"[package]
name = "cranelift-json-quoted-runtime-format-output-main-exit"
version = "0.1.0"
...
[unsafe_rationale]
stdio = "Direct-native stdout/stderr regression covers JSON-quoted runtime formatted output for issue 1001."
"#,
    )
    .expect("write JSON-quoted runtime format output manifest");
    fs::write(
        project.join("axiom.lock"),
        r#"version = 1
...
"#,
    ...
    .expect("write JSON-quoted runtime format output lockfile");
    fs::write(
        project.join("src/main.ax"),
        r#"import "std/io.ax"
import "std/json.ax"
...
let total_text: string = stringify_int(total)
let enabled_text: string = stringify_bool(enabled)
let quoted_total_text: string = stringify_string(total_text)
let quoted_enabled_text: string = stringify_string(enabled_text)
let quoted_enabled_clone: string = string_clone(quoted_enabled_text)
print quoted_total_text
let written: int = eprintln(quoted_enabled_clone)
if total == 21 && enabled && written == 7 {
return 48
} else {
return 1
}
...
fn write_runtime_bool_stdout_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src")).expect("create runtime bool stdout main project src");
    fs::write(
        project.join("axiom.toml"),
        r#"[package]
name = "cranelift-runtime-bool-stdout-main-exit"
version = "0.1.0"
...
[unsafe_rationale]
stdio = "Direct-native stdout regression covers runtime bool print statements for issue 1001."
"#,
    )
    .expect("write runtime bool stdout main manifest");
    fs::write(
        project.join("axiom.lock"),
        r#"version = 1
...
"#,
    ...
    .expect("write runtime bool stdout main lockfile");
    fs::write(
        project.join("src/main.ax"),
        r#"fn main(): int {
let enabled: bool = false
...
"#,
    ...
    .expect("write runtime bool stdout main source");
}

fn write_runtime_int_stdout_main_exit_project(project: &Path) {
    fs::create_dir_all(project.join("src")).expect("create runtime int stdout main project src");
    fs::write(
        project.join("axiom.toml"),
        r#"[package]
name = "cranelift-runtime-int-stdout-main-exit"
version = "0.1.0"
...
[unsafe_rationale]
stdio = "Direct-native stdout regression covers runtime int print statements for issue 1001."
"#,
    )
    .expect("write runtime int stdout main manifest");
    fs::write(
        project.join("axiom.lock"),
        r#"version = 1
...
"#,
    ...
    .expect("write runtime int stdout main lockfile");
    fs::write(
        project.join("src/main.ax"),
        r#"fn main(): int {
let total: int = 0
...
let negative: int = 0 - total
print total
print negative
if total == 21 && negative == -21 {
return 48
} else {
return 1
}
