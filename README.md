...
# ./registry is a checked-in quickstart fixture (genuine publish/registry-index output).
# Regenerate it via scripts/registry-fixtures/generate-registry-fixtures.sh; the
# "Registry Quickstart Fixtures" CI lane keeps the checked-in bytes honest.
...
31 commands, 34 stdlib modules with 307 exported functions, and 9 manifest
...
- `registry/`: checked-in quickstart signed-registry fixtures, regenerated by
  `scripts/registry-fixtures/generate-registry-fixtures.sh` and kept honest by
  the `Registry Quickstart Fixtures` CI lane.
